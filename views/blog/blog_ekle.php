...
<html lang="en" class="light-style layout-navbar-fixed layout-menu-fixed layout-compact" dir="ltr" data-theme="theme-default" data-assets-path="/assets/" data-template="vertical-menu-template" data-style="light">
    <head>
        ...
        <title> <?=$row_site->TITLE?> | Blog Ekle </title>
        <?=$cTheme->Linkler()?>
    </head>
    <body>
                                        ...
                                        <div class="card-header overflow-hidden">
                                            <ul class="nav nav-tabs" role="tablist">
                                                <li class="nav-item" role="presentation">
                                                    <button class="nav-link waves-effect active" data-bs-toggle="tab" data-bs-target="#tab_stok" role="tab" aria-selected="true">
                                                    <span class="ri-user-line ri-20px d-sm-none"></span><span class="d-none d-sm-block">Blog Bilgisi</span></button>
                                                </li>
                                            </ul>
                                        </div>
                                        <div class="tab-content">
                                            <div class="tab-pane fade active show" id="tab_stok" role="tabpanel">
                                                <form id="blogEkle">
                                                    <div class="row g-6">
                                                        <div class="col-md-6">
                                                            <div class="input-group input-group-merge">
                                                                ...
                                                                <div class="form-floating form-floating-outline">
                                                                    <input type="text" id="ad" name="ad" class="form-control" value="<?=$_REQUEST['ad']?>" placeholder="Blog Adı">
                                                                    <label>Adı</label>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <div class="input-group input-group-merge">
                                                                ...
                                                                <div class="form-floating form-floating-outline">
                                                                    <input type="text" id="baslik" name="baslik" class="form-control" value="<?=$_REQUEST['baslik']?>" placeholder="Blog Başlık">
                                                                    <label>Başlık</label>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <div class="input-group input-group-merge">
                                                                ...
                                                                <div class="form-floating form-floating-outline">
                                                                    <input type="text" id="url" name="url" class="form-control" value="<?=$_REQUEST['url']?>" placeholder="URL">
                                                                    <label>URL</label>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="col-md-12">
                                                            <div class="input-group input-group-merge">
                                                                ...
                                                                <div class="form-floating form-floating-outline">
                                                                    <textarea class="form-control" id="aciklama" name="aciklama"  placeholder="Açıklama" value="<?=$_REQUEST['aciklama']?>" style="height: 81px;"></textarea>
                                                        ...
                                                        <div class="col-md-12">
                                                            <div id="icerik"></div>
                                                            <input type="hidden" name="icerik" id="hidden-input">
                                                        </div>
                                                    </div>
...
<script type="text/javascript">
    ...
    $("#ad").on("change", function(event) {
        const title = this.value;
        const turkishMap = {'ç': 'c', 'Ç': 'C','ğ': 'g', 'Ğ': 'G','ı': 'i', 'I': 'I','İ': 'i', 'ö': 'o','Ö': 'O', 'ş': 's','Ş': 'S', 'ü': 'u','Ü': 'U'};

        const slug = title
            .toLowerCase() // Küçük harfe çevir
            .replace(/[\u00C0-\u017F]/g, c => turkishMap[c] || c) // Türkçe karakterleri dönüştür
            .replace(/[^a-z0-9\s-]/g, '') // Özel karakterleri kaldır
            .replace(/\s+/g, '-') // Boşlukları `-` ile değiştir
            .replace(/-+/g, '-') // Fazladan `-` işaretlerini kaldır
            .replace(/^-|-$/g, ''); // Başta veya sonda `-` varsa kaldır

        $("#url").val(slug)
    });

    $("#blogEkle").on("submit", function(event) {
        event.preventDefault();
        showSpinner();
        $.ajax({
            url: "/router.php",
            type: "POST",
            data: $(this).serialize() + "&controller=blog&action=blog_ekle",
            dataType: "json",
